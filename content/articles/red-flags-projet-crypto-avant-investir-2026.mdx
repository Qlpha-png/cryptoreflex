...
description: "Equipe anonyme, pas d'audit, liquidité faible, influenceurs payés... Identifie les red flags d'un projet crypto risqué avant de mettre un euro."
...
Cet article porte sur l'évaluation d'un projet potentiellement légitime mais risqué. Si tu cherches comment reconnaître une fraude explicite (rug pull, schéma de Ponzi, pig butchering), le [parcours Arnaques de l'Académie](/academie/arnaques) est fait pour ça.
</Callout>
...
Dès que tu lis ou entends le mot "garanti" associé à un rendement en crypto, arrête tout. Ce n'est pas un signal d'alerte parmi d'autres : c'est une ligne rouge. Passe ton chemin.
</Callout>
...
Avant tout investissement dans un projet inconnu, pose ces quatre questions en moins de 10 minutes : (1) L'équipe est-elle identifiable avec un historique vérifiable ? (2) Existe-t-il un rapport d'audit complet accessible ? (3) La liquidité est-elle verrouillée avec une durée significative ? (4) Le code est-il public et actif sur GitHub ? Si tu ne peux pas répondre oui aux quatre, passe ton chemin.
</Callout>
...
Pour approfondir les techniques d'analyse d'un projet, consulte notre guide sur l'[analyse on-chain pour débutants](/academie) et notre article sur la [capitalisation, le volume et la liquidité en crypto](/outils/glossaire-crypto).
...
- La concentration des tokens sur quelques portefeuilles et une liquidité non verrouillée créent un risque structurel de manipulation : vérifie-les sur l'explorateur blockchain et sur DEXTools avant d'investir.
...
- Pour les arnaques explicites (rug pull, Ponzi, pig butchering), consulte le [parcours Arnaques de l'Académie](/academie/arnaques) qui couvre les mécanismes de fraude en détail.
</KeyTakeaways>
