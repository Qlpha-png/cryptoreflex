...
excerpt: "La passphrase BIP39 — surnommée le 25e mot — transforme vos 24 mots en un nombre infini de wallets cachés. Ultra-sécurisé contre le wrench attack, mais 1 typo = fonds perdus à jamais. Voici comment l'activer correctement."
...
Vous avez configuré votre Ledger, vous avez sauvegardé votre seed phrase de 24 mots sur plaque acier, et vous vous demandez maintenant comment **passer au niveau supérieur** de sécurité. La réponse a un nom : la **passphrase BIP39**, surnommée à tort le **25e mot**. C'est la fonctionnalité la plus puissante et la plus mal comprise des hardware wallets.
...
- **Passphrase BIP39** = mot ou phrase supplémentaire (de 1 à 100 caractères) qui se combine à vos 24 mots pour générer un wallet **différent**.
- **24 mots seuls** → wallet visible standard. **24 mots + passphrase** → wallet caché complètement séparé.
- **Avantage** : protection ultime contre le wrench attack (vous donnez les 24 mots sous la torture, l'attaquant tombe sur votre wallet "leurre" presque vide).
- **Risque mortel** : perdre la passphrase = fonds du wallet caché perdus définitivement, même avec les 24 mots intacts.
- **Pour qui ?** À partir de 20 000 € en crypto, ou si vous vivez dans un pays à risque physique élevé.
</Callout>
...
1. Vos 24 mots sont d'abord convertis en une **entropie binaire** (256 bits + 8 bits de checksum).
...
3. La passphrase sert de **sel** dans cette fonction PBKDF2. Sans passphrase, le sel par défaut est la chaîne "mnemonic". Avec passphrase, le sel devient "mnemonic" + votre passphrase.
...
Si votre wallet leurre est **complètement vide**, l'attaquant ne vous croira pas : il sait que vous avez forcément quelque chose. Gardez **20 à 500 €** crédibles sur le wallet leurre — assez pour paraître réel, pas assez pour vous appauvrir si vous le perdez. C'est la dîme de sécurité.
</Callout>
...
1. Ledger Live → **Mes apps** → ouvrez l'app **Bitcoin** sur l'appareil.
2. Sur le Ledger physiquement, allez dans **Settings** → **Security** → **Passphrase**.
3. Choisissez **Set up passphrase** → **Attach to PIN**.
4. Vous choisissez un **second PIN** (différent du PIN principal). Ce second PIN active automatiquement la passphrase quand vous l'utilisez pour déverrouiller.
5. Saisissez votre passphrase sur l'appareil (clavier virtuel via les boutons). Long, mais c'est l'option la plus sécurisée.
6. L'appareil vous demande de confirmer. Une fois validée, votre passphrase est enregistrée.
...
Pour suivre la procédure officielle Ledger avec captures d'écran, consultez la <a href="https://support.ledger.com/article/4404366864-zd" target="_blank" rel="noopener noreferrer">documentation Ledger Academy sur la passphrase</a>. Si vous n'avez pas encore de hardware wallet, le <AffiliateLink href="https://shop.ledger.com/" platform="ledger" placement="article-passphrase-tuto">Ledger Nano X</AffiliateLink> supporte nativement la passphrase BIP39 depuis 2018, avec une UX dédiée (PIN secondaire). C'est la référence pour ce setup.
</Callout>
...
Pour les patrimoines importants, vous pouvez **diviser votre passphrase via Shamir** (SLIP-39) : 3 parts dont 2 suffisent pour reconstruire. Une part chez vous, une chez un proche, une au coffre bancaire. Voir notre guide [Shamir backup](/blog/shamir-backup-seed-decouper-en-parts-2026).

<Callout type="warning" title="Erreur classique : la passphrase oubliée à 99 %">
"Je m'en souviendrai" est l'illusion qui tue le plus de wallets. Une passphrase utilisée tous les 6 mois s'oublie en 18 à 36 mois pour 80 % des gens. **Vous DEVEZ avoir un backup physique** (sauf si vous acceptez consciemment le risque de perte). Et vous devez **tester** la récupération une fois par an : restaurez votre wallet sur un Ledger neuf avec les 24 mots + passphrase, vérifiez que les fonds apparaissent.
</Callout>
...
1. **Activer une passphrase sans test de récupération immédiat** : vous activez, vous envoyez 50 000 € sur le wallet caché, vous vous cassez le bras, vous oubliez la passphrase précise → tout perdu. **Toujours tester** la restauration sur un Ledger vierge avant de transférer des montants importants.
...
4. **Wallet leurre à 0 €** : alerte immédiate l'attaquant. Gardez au moins 100-300 € crédibles dessus.
5. **Utiliser une passphrase facile à deviner** (date de naissance, nom du chien, citation de film) : un attaquant qui vous cible peut brute-forcer en quelques heures.

## Pour aller plus loin

- [Configurer votre Ledger Nano X en 30 minutes](/blog/configurer-ledger-nano-x-30-minutes-guide-pas-a-pas) — la base avant la passphrase.
...
<FAQ items={[
  {
    ...
    answer: "Oui à 100 %. La passphrase BIP39 est un standard ouvert. Vous pouvez configurer une passphrase sur Ledger, perdre votre appareil, racheter un Trezor (ou un Coldcard), restaurer avec les 24 mots + la même passphrase, et vous retrouvez exactement les mêmes adresses et fonds. C'est l'avantage des standards BIP."
  },
  {
...
<KeyTakeaways>
- La passphrase se combine à vos mots de récupération pour générer un wallet caché totalement séparé du wallet visible.
- Son intérêt majeur est la négation plausible face au wrench attack : vous pouvez exposer un wallet leurre crédible et garder l'essentiel invisible.
- Retenez le risque mortel : une passphrase oubliée ou mal orthographiée entraîne la perte définitive des fonds, même avec vos mots intacts.
- Sauvegardez votre passphrase sur un support séparé de votre seed, et testez la récupération avant d'y transférer un montant important.
- Gardez un peu de fonds crédibles sur le wallet leurre : complètement vide, il alerte immédiatement un attaquant.
</KeyTakeaways>
